...
class Manual_invoice_model extends CI_Model
{
    ...
    public function update($id, $header, $items)
    {
        ...
        foreach ((array)$items as $it) {
            ...
            $total_tax_amount += $tax_amount;
            $grand_total += $amount;
            $clean_items[] = compact('description','qty','unit','unit_price','discount_percent','tax_percent','subtotal','discount_amount','tax_amount','amount');
        }
        ...
        $header['total_tax_amount'] = $total_tax_amount;
        $header['paid_amount'] = $paid_amount;
        $header['balance_amount'] = $grand_total - $paid_amount;
        $header['total_amount'] = $grand_total;

        $this->db->where('id', $id)->update($this->table, $header);
